...
def plot_PSG_E4(segmented_df, features):
    segmented_df["Sleep_Stage"] = segmented_df["Sleep_Stage"].map(
        {"N1": 0, "N2": 0, "N3": 0, "R": 1, "W": 2}
    )
    # Create figure and GridSpec with uneven row heights
    fig = plt.figure(figsize=(14, len(features) * 2 + 2))
    gs = GridSpec(
    ...
    ax3.plot(
        segmented_df["TIMESTAMP"],
        segmented_df["Obstructive_Apnea"],
        color="red",
        linestyle="none",
        marker="o",
        markersize=4,
